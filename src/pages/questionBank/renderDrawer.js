...
class RenderDrawer extends React.Component {
  ...
  removeAll = () => {
    this.props.dispatch({
      type: "questionGenerator/removeAllTestPaperGenList"
    });
  };
          ...
          <Collapse accordion expandIconPosition="right">
            {this.props.testPaperGenList.map((item, index) => {
              return (
                <Collapse.Panel
                  header={`${index + 1}、${
                    item.topic.toString().length > 40
                      ? item.topic.toString().slice(0, 40) + "..."
                      : item.topic.toString()
                  }`}
                  key={index}
                >
                  ...
                  >
                    ...
                    <Descriptions.Item label="题目类型">
                      {renderTopicType(item.topic_type)}
                    </Descriptions.Item>
                    <Descriptions.Item label="分值">
                      {item.score}
                    </Descriptions.Item>
                    <Descriptions.Item label="大知识点">
                      {item.chapter_1}
                    </Descriptions.Item>
                    <Descriptions.Item label="小知识点">
                      {item.chapter_2}
                    </Descriptions.Item>
                    <Descriptions.Item label="小章节">
                      {item.label_1}
                    </Descriptions.Item>
                    <Descriptions.Item label="大章节">
                      {item.label_2}
                    </Descriptions.Item>
