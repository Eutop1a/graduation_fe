...
class QuestionBank extends React.Component {
          ...
          render: record => (
            <div>
              {this.isInclude(record) ? (
                <Button
                  type="link"
                  onClick={this.handleRemoveTestPaperGenList.bind(this, record)}
                >
                  移出
                </Button>
              ) : (
                <Button
                  type="link"
                  onClick={this.handleAddTestPaperGenList.bind(this, record)}
                >
                  选择!
                </Button>
              )}
              <Button
                type="link"
                onClick={this.handleEditTopic.bind(this, record)}
              ...
              <Popconfirm
                ...
                disabled={this.isInclude(record)}
              >
                <Button type="link" disabled={this.isInclude(record)} danger>
                  删除
                </Button>
              </Popconfirm>
            </div>
  ...
  handleAddTestPaperGenList = async record => {
    await this.props.dispatch({
      type: "questionGenerator/addTestPaperGenList",
      payload: record
    });
  };
  handleRemoveTestPaperGenList = async record => {
    await this.props.dispatch({
      type: "questionGenerator/removeTestPaperGenList",
      payload: record
    });
  };
  ...
  handleDeleteTopic = async record => {
    ...
    await this.props.dispatch({
      type: "questionBank/deleteSingleQuestionBank",
  ...
  render() {
    const renderTable = () => {
      return (
        <div className={style.table_wrapper}>
          ...
          >
            {this.state.columnTitle.map((item, index) => (
    ...
    return (
      <div>
        ...
        <PageHeader
          title={
            "题库管理（当前共" + this.props.tableDataSource.length + "题）"
          }
        ...
        <OverViewModal
          ...
          hide={this.hideModal}
          dataSource={this.props.tableDataSource}
          eachChapterCount={this.props.eachChapterCount}
        />
        {/*题库表格*/}
        {renderTable()}
      </div>
