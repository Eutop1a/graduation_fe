...
export default class MainTable extends React.Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: record => (
          <div>
            <Button type="link" onClick={this.getReport.bind(this, record)}>
              查看详情
            </Button>
            <Dropdown
              overlay={
                <Menu onClick={this.reExport.bind(this, record)}>
                  <Menu.Item key="1">
                    <Button type="link">试卷(*.docx)</Button>
                  </Menu.Item>
                  <Menu.Item key="2">
                    <Button type="link">答案(*.docx)</Button>
                  </Menu.Item>
                </Menu>
              }
              placement="bottom"
              arrow
            >
              <Button type="link" onClick={this.reExport.bind(this, record)}>
                重新导出
              </Button>
            </Dropdown>
            <Button
              type="link"
              onClick={this.changeModifyTestPaperVisible.bind(this, record)}
            >
              修改组卷
            </Button>
  ...
  getReport = record => {
    this.props.dispatch({
      type: "questionGenHistory/reportDifficulty",
      payload: record.average_difficulty
    });
  ...
  changeModifyTestPaperVisible = async record => {
    ...
    await this.setState({
      modifyTestPaperVisible: !this.state.modifyTestPaperVisible
    });
    await this.props.dispatch({
      type: "questionGenHistory/getAllTestPaperGenHistory"
    });
  };
  ...
  render() {
    const renderTable = () => {
      return (
        <Table
          ...
          loading={
            this.state.loading
              ? { size: "large", indicator: <LoadingOutlined /> }
              : false
          }
          bordered
          style={{ margin: "0 30px" }}
    ...
    return (
      <div>
        {renderTable()}
        <ModifyTestPaper
          visible={this.state.modifyTestPaperVisible}
