...
class ModifyTestPaper extends React.Component {
  ...
  async getQuestions(test_paper_uid) {
    ...
    await this.props.dispatch({
      type: "questionGenHistory/getQuestionGenHistoriesByTestPaperUid",
      payload: { test_paper_uid: test_paper_uid }
    });
    // 返回的结果
    await this.setState({
      questions: this.props.testPaperReportQuestionList ?? []
    });
    await delay(1000);
  ...
  async getTopicType() {
    ...
    await this.setState({
      selectedTopicType:
        this.state.topicType.length > 0 ? this.state.topicType[0] : ""
    });
  }
  ...
  async addBtn(index) {
    this.state.questions.splice(index + 1, 0, {
      new_flag: true,
      create_done: false,
      index: Math.random()
    });
  ...
  render() {
    ...
    const renderAllQuestion = () => {
      return this.state.questions.map((value, index) => {
        ...
        const style = () => {
          // 如果被删除了就显示删除线
          if (value.deleted_flag && !value.new_flag)
            return {
              textDecoration: "line-through",
              color: "#AAA",
              borderLeft: "0.35rem solid #CCC"
            };
          // 如果是新增的题目，就标绿
          if (!value.deleted_flag && value.new_flag && value.create_done)
            return { borderLeft: "0.35rem solid #5cb85c" };
        };
        const delBtn = !value.deleted_flag ? (
          <Button
            onClick={this.delBtn.bind(this, index)}
            type="link"
            danger
            icon={<DeleteOutlined />}
          >{`删除`}</Button>
          ...
          <Button
            onClick={this.cancelDelBtn.bind(this, index)}
            type="link"
            icon={<UndoOutlined />}
          >{`恢复`}</Button>
        );
        ...
        return (
          <div key={index + 1}>
            ...
            <Divider dashed orientation="right">
              <Button
                type="link"
                onClick={this.addBtn.bind(this, index)}
    ...
    return (
      <Modal
        title={`修改组卷 《${this.props.record?.test_paper_name}》`}
        centered={true}
      ...
      >
        ...
        >
          <h1 style={{ textAlign: "center", fontSize: "1.4rem" }}>
            {this.props.record?.test_paper_name}.docx
          </h1>
          <p style={{ fontSize: "0.75rem", color: "#888", textAlign: "right" }}>
            组卷时间：
            {moment(this.props.record?.update_time).format(
              "YYYY-MM-DD HH:mm:ss"
            )}
          </p>
...
class SearchTopic extends React.Component {
  ...
  searchTopic = async value => {
    const asyncRequest = async () => {
      const payload = {
        keyword: value,
        topicType: this.state.selectedTopicType
      };
      const response = await searchQuestionByTopic(payload);
      response.data instanceof Array &&
        this.setState({ searchResult: response.data });
  ...
  render() {
    // 题目类型选择框
    const options_1 = this.state.topicType.map(value => (
      <Select.Option key={value}>{value}</Select.Option>
    ));
    const changeSelectedValue = value =>
      this.setState({ selectedTopicType: value });
    // 搜索结果
    const options_2 = this.state.searchResult.map((value, index) => {
      return {
        question: value,
        topicIndex: this.props.index,
        value: `（本题${value.score}分）${value.topic}`,
        label: `${value.topic}`
      };
    });
    ...
    return (
      <div style={{ display: "flex", flexWrap: "nowrap" }}>
        ...
        <Select
          value={this.state.selectedTopicType}
        ...
        ></Button>
        <Button
          onClick={this.props.onCancel.bind(this, this.props.index)}
          type="danger"
          style={{ marginLeft: "5px" }}
          icon={<CloseOutlined />}
        ></Button>
