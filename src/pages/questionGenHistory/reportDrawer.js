...
class ReportDrawer extends React.Component {
  ...
  render() {
    return (
      ...
      >
        <PageHeader
          title={`${this.getTestPaperName()}试卷出题历史记录`}
          subTitle={"详细记录"}
        />

        <Divider orientation="left" style={{ fontWeight: "bold" }}>
          总体难度{this.props.reportDifficulty}
        </Divider>
        <div className={style.flex_middle}>
          <span className={style.text_two_side_padding}>难度1</span>
          <Progress
            percent={((this.props.reportDifficulty / 5) * 100).toFixed(2)}
        ...
        <Divider orientation="left" style={{ fontWeight: "bold" }}>
          本试卷共{this.props.testPaperReportQuestionList.length}题
        </Divider>
          ...
          ? this.props.testPaperReportQuestionList.map((item, index) => {
              return (
                <Descriptions span={1} key={index}>
                  <Descriptions.Item
                    contentStyle={{ fontSize: "0.8em", color: "gray" }}
                  >
                    <Tag color="magenta">
