  ...
  effects: {
    *getAllQuestionLabels({ payload }, { call, put }) {
      try {
        ...
        if (checkCode(res) && isArray(res.data)) {
          ...
          res.data.forEach(item => {
            ...
            if (chapter1.indexOf(item.chapter_1) === -1)
              chapter1.push(item.chapter_1);
            if (chapter2.indexOf(item.chapter_2) === -1)
              chapter2.push(item.chapter_2);
    ...
    filterChapter2ByChapter1: function*({ payload }, { select, put }) {
      let tmpArray = [];
      const allQuestionLabels = yield select(
        state => state.questionEdit.allQuestionLabels
      );
      allQuestionLabels.forEach(each => {
        if (
          each.chapter_1 === payload &&
          tmpArray.indexOf(each.chapter_2) === -1
        )
          tmpArray.push(each.chapter_2);
      });
    ...
    filterLabel1ByChapter1: function*({ payload }, { select, put }) {
      ...
      allQuestionLabels.forEach(each => {
        if (each.chapter_1 === payload && tmpArray.indexOf(each.label_1) === -1)
          tmpArray.push(each.label_1);
      });
    ...
    filterLabel2ByChapter1: function*({ payload }, { select, put }) {
      ...
      allQuestionLabels.forEach(each => {
        if (each.chapter_1 === payload && tmpArray.indexOf(each.label_2) === -1)
          tmpArray.push(each.label_2);
      });
    ...
    *uploadFile({ payload }, { call, put }) {
      try {
        ...
        if (checkCode(res)) {
          message.success(
            `成功导入${res.data.insertCount}条，删除了原有的${res.data.deleteCount}条.`,
            3
          );
        }
      } catch (e) {
