...
export default {
  ...
  effects: {
    addTestPaperGenList: function*({ payload }, { select, put }) {
      const testPaperGenList = yield select(
        state => state.questionGenerator.testPaperGenList
      );
      if (!testPaperGenList.includes(payload)) {
        message.success("添加成功");
        yield put({
          type: "testPaperGenList",
          payload: [...testPaperGenList, payload]
        });
      } else {
        message.warning("冗余");
      }
    ...
    removeTestPaperGenList: function*({ payload }, { select, put }) {
      ...
      if (!testPaperGenList.includes(payload)) {
        ...
        yield put({
          type: "testPaperGenList",
          payload: testPaperGenList.filter(item => {
            return item !== payload;
          })
        });
      }
    ...
    geneticSelect: function*({ payload }, { select, call, put }) {
      try {
        ...
        if (checkCode(res)) {
          console.log("Genetic Select Response:", res.data);
          console.log("Variance Data:", res.data.variance);
          yield put({ type: "TKTList", payload: res.data.TKTList });
          yield put({ type: "PDTList", payload: res.data.PDTList });
          yield put({ type: "JDTList", payload: res.data.JDTList });
          yield put({ type: "XZTList", payload: res.data.XZTList });
    ...
    questionGen: function*({ payload }, { select, call, put }) {
      try {
        ...
        if (payload.randomSwitch) {
          // 填空题Ids
          const TKTList = yield select(
            state => state.questionGenerator.TKTList
          );
          let TKTIdList = [];
          isArray(TKTList)
            ? TKTList.forEach(item => {
                TKTIdList.push(item.id);
              })
            : null;
          postPayload["TKTIdList"] = TKTIdList;
          // 选择题Ids
          const XZTList = yield select(
            state => state.questionGenerator.XZTList
          );
          let XZTIdList = [];
          isArray(XZTList)
            ? XZTList.forEach(item => {
                XZTIdList.push(item.id);
              })
            : null;
          postPayload["XZTIdList"] = XZTIdList;
          // 判断题Ids
          const PDTList = yield select(
            state => state.questionGenerator.PDTList
          );
          let PDTIdList = [];
          isArray(PDTList)
            ? PDTList.forEach(item => {
                PDTIdList.push(item.id);
              })
            : null;
          postPayload["PDTIdList"] = PDTIdList;
          // 简答题Ids
          const JDTList = yield select(
            state => state.questionGenerator.JDTList
          );
          let JDTIdList = [];
          isArray(JDTList)
            ? JDTList.forEach(item => {
                JDTIdList.push(item.id);
              })
            : null;
